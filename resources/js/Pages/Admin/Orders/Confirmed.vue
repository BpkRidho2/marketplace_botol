...
<template>
    <Head title="Pesanan Dikonfirmasi" />

    <AdminLayout>
        <div class="space-y-6">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Pesanan Dikonfirmasi</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Pesanan yang sudah dikonfirmasi dan sedang diproses</p>
            </div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="(order, index) in orders.data" :key="order.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 uppercase">
                                        Confirm
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex gap-2 flex-col">
                                        ...
                                        <button 
                                            @click="editingStatusId = editingStatusId === order.id ? null : order.id"
                                            class="text-blue-600 hover:text-blue-800 font-semibold text-sm">
                                            {{ editingStatusId === order.id ? 'Batal' : 'Ubah Status' }}
                                        </button>
                                        <select v-if="editingStatusId === order.id"
                                                class="px-2 py-1 text-sm border rounded dark:bg-gray-600 dark:text-gray-100"
                                                @change="(e) => {
                                                    if (e.target.value) {
            ...
            <div v-else class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-12 text-center">
                ...
                <p class="mt-4 text-gray-500 dark:text-gray-400">Tidak ada pesanan yang dikonfirmasi</p>
            </div>
        </div>
    </AdminLayout>
</template>
